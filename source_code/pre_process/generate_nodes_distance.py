...
def generate_distances(benign_data, node1, node2, distance_data_rows):
    ...
    temp_df = pd.DataFrame({'NODE_1': [node1, node2], 'LAT_1': [data1['LAT'], data2['LAT']],
                            'LNG_1': [data1['LNG'], data2['LNG']],
                            'NODE_2': [node2, node1], 'LAT_2': [data2['LAT'], data1['LAT']],
...
def main_generate_distances(time_step, num_nodes, group_number):
    ...
    output_path = CONFIG.OUTPUT_DIRECTORY + 'pre_process/Output/group_' + str(group_number) + '/metadata/distances.csv'
    prepare_output_directory(output_path)

    distance_data = pd.DataFrame(columns=['NODE_1', 'LAT_1', 'LNG_1', 'NODE_2', 'LAT_2', 'LNG_2', 'DISTANCE'])

    manager = Manager()
    distance_data_rows = manager.list([distance_data])

    p = Pool(30)
    p.starmap(generate_distances, product([benign_dataset], nodes, nodes, [distance_data_rows]))
    p.close()
    p.join()
