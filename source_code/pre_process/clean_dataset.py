...
def create_benign_dataset(data, begin_date, end_date, time_step, nodes, benign_packet_cauchy, output_path):
...
def main_generate_benign_data(time_step):
    ...
    for index in range(num_groups):
        ...
        benign_data_output_path = CONFIG.OUTPUT_DIRECTORY + 'pre_process/Output/group_' + \
                                  str(index) + '/benign_data/benign_data_' +  str(begin_date) + '_' + str(end_date) + \
                                  '_time_step_' + str(time_step) + '_num_ids_' + str(num_nodes_per_group) + '.csv'
        prepare_output_directory(benign_data_output_path)
        create_benign_dataset(original_data, begin_date, end_date, time_step, selected_nodes, benign_packet_cauchy,
