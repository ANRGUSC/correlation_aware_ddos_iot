...
def main_generate_attack(dataset_directory_path, group_number, data_type, num_train_days, num_validation_days, num_test_days,
                         k_list, attacked_ratio_nodes_list, attack_duration_list, attack_start_times_list,
                         time_step, num_nodes):
    ...
    benign_dataset_path = dataset_directory_path + 'group_' + str(group_number) + '/benign_data/' + \
    ...
    output_path = dataset_directory_path + 'group_' + str(group_number) + '/attacked_data/' + data_type + '/'
    ...
    benign_packet_path = CONFIG.DATASET_DIRECTORY + 'N_BaIoT/SimpleHome_XCS7_1003_WHT_Security_Camera_benign_aggregation_Source-IP_time_window_10-sec_stat_Number.csv'
    ...
    attack_packet_path = CONFIG.DATASET_DIRECTORY + 'N_BaIoT/SimpleHome_XCS7_1003_WHT_Security_Camera_mirai_attack_udp_aggregation_Source-IP_time_window_10-sec_stat_Number.csv'
    ...
    for k in k_list:
        ...
        p.starmap(generate_attack, product([benign_data], [attack_begin_date], [attack_end_date],
                                                 attacked_ratio_nodes_list, attack_duration_list, attack_start_times_list,
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        attack_start_times_list = [timedelta(hours=float(i)) for i in attack_start_times_list]



    for group_number in range(CONFIG.NUM_GROUPS):
        main_generate_attack(dataset_directory_path, group_number, 'train', num_train_days, num_validation_days,
                             num_test_days, k_list, attacked_ratio_nodes_list, attack_duration_list,
                             attack_start_times_list, time_step, num_nodes)
        print('generate train data done.')
        main_generate_attack(dataset_directory_path, group_number, 'validation', num_train_days, num_validation_days,
                             num_test_days, k_list, attacked_ratio_nodes_list, attack_duration_list,
                             attack_start_times_list, time_step, num_nodes)
        print('generate validation data done.')
        main_generate_attack(dataset_directory_path, group_number, 'test', num_train_days, num_validation_days,
                             num_test_days, k_list, attacked_ratio_nodes_list, attack_duration_list,
                             attack_start_times_list, time_step, num_nodes)
