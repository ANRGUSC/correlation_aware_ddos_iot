...
def add_other_nodes_packets(fname, num_days, combined_data_rows):
    ...
    for node_1 in nodes:
        ...
        for node_2 in nodes:
            ...
            data.loc[data['NODE'] == node_1, 'PACKET_'+str(node_2)] = data_node_2['PACKET'].values
...
def combine_data(num_days, input_path, output_path):
    ...
    columns = ['BEGIN_DATE', 'END_DATE', 'NUM_NODES', 'ATTACK_RATIO', 'ATTACK_START_TIME', 'ATTACK_DURATION',
               'ATTACK_PARAMETER', 'NODE', 'LAT', 'LNG', 'TIME', 'TIME_HOUR', 'ACTIVE', 'PACKET', 'ATTACKED']
    for node in nodes:
