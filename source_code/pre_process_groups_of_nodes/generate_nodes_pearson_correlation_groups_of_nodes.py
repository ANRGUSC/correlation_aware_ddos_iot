...
def generate_correlations(benign_data, node1, node2, correlation_data_rows):
    ...
    temp_df = pd.DataFrame({'NODE_1': [node1, node2], 'LAT_1': [data1['LAT'][0], data2['LAT'][0]],
                            'LNG_1': [data1['LNG'][0], data2['LNG'][0]],
                            'NODE_2': [node2, node1], 'LAT_2': [data2['LAT'][0], data1['LAT'][0]],
                            'LNG_2': [data2['LNG'][0], data1['LNG'][0]],
                            'CORRELATION': [correlation, correlation]})
    correlation_data_rows.append(temp_df)
    # correlation_data_rows = pd.concat([correlation_data_rows, temp_df])
...
def main_generate_correlations(group_number):
    ...
    'pre_process/Output/group_' + str(group_number) + '/benign_data/benign_data_2021-01-02 00:00:00_2021-02-01 23:59:58_time_step_' + \
    ...
    output_path = CONFIG.OUTPUT_DIRECTORY + 'pre_process/Output/group_' + str(group_number) + '/metadata/correlations.csv'
    prepare_output_directory(output_path)

    correlation_data = pd.DataFrame(columns=['NODE_1', 'LAT_1', 'LNG_1', 'NODE_2', 'LAT_2', 'LNG_2', 'CORRELATION'])

    manager = Manager()
    correlation_data_rows = manager.list([correlation_data])

    p = Pool(30)
    p.starmap(generate_correlations, product([benign_dataset], nodes, nodes, [correlation_data_rows]))
    p.close()
    p.join()
