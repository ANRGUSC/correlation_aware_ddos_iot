...
def combine_general_reports(input_path, metadata_metric_list, nn_model_list, architecture_list, metric, mode,
                            output_directory):
        ...
        for nn_model in nn_model_list:
            for architecture in architecture_list:
                for group_number in range(81):
                    general_report_path = input_path + 'group_' + str(group_number) + '/metadata_' + metadata_metric + \
                                          '/' + nn_model + '/' + architecture + '/run_0/report/metrics_evaluation/' + \
                                          'data/general_report_' + metric + '_' + mode + '.csv'
                    tmp_df = pd.read_csv(general_report_path)
                    general_report_df = pd.concat([general_report_df, tmp_df])
                output_path = output_directory + 'metadata_' + metadata_metric + \
                              '/' + nn_model + '/' + architecture + '/'
                prepare_output_directory(output_path)
                output_path_general_report = output_path + 'general_report_combined_' + \
                                             metric + '_' + mode + '.csv'
                ...
                output_path_mean_report = output_path + 'mean_report_combined_' + \
                                          metric + '_' + mode + '.csv'
                mean_report_df.to_csv(output_path_mean_report, index=False)


def combine_roc_reports(input_path, metadata_metric_list, nn_model_list, architecture_list, metric, mode,
                        output_directory):
    ...
    for metadata_metric in metadata_metric_list:
        for nn_model in nn_model_list:
            for architecture in architecture_list:
                for group_number in range(81):
                    roc_report_path = input_path + 'group_' + str(group_number) + '/metadata_' + metadata_metric + \
                                          '/' + nn_model + '/' + architecture + '/run_0/report/roc/' + \
                                          'data/roc_clean_report_' + metric + '_' + mode + '.csv'
                    tmp_df = pd.read_csv(roc_report_path)
                    roc_report_df = pd.concat([roc_report_df, tmp_df])
                output_path = output_directory + 'metadata_' + metadata_metric + \
                              '/' + nn_model + '/' + architecture + '/'
                prepare_output_directory(output_path)
                output_path_roc_report = output_path + 'roc_report_combined_' + \
                                             metric + '_' + mode + '.csv'
                ...
                output_path_roc_mean_report = output_path + 'roc_mean_report_combined_' + \
                                          metric + '_' + mode + '.csv'
...
def main_combine_reports():
    ...
    mode = 'max'
    metadata_metric_list = ['NOT_USED', 'CORRELATION']
    nn_model_list = ['lstm']
    architecture_list = ['multiple_models_with_correlation']
    combine_general_reports(input_path, metadata_metric_list, nn_model_list, architecture_list, metric, mode,
                            output_directory)
    combine_roc_reports(input_path, metadata_metric_list, nn_model_list, architecture_list, metric, mode,
                        output_directory)
...
def main_plot_compare_current(metric, mode):
    ...
    for nn_model in nn_model_list:
        ...
        for architecture in architecture_list:
            input_path = CONFIG.OUTPUT_DIRECTORY + 'nn_model_analysis/data/' + 'metadata_' + metadata_metric + '/' + \
                         nn_model + '/' + architecture + '/'
            ...
            roc_mean_report_path = input_path + 'roc_mean_report_combined_' + metric + '_' + mode + '.csv'
            roc_mean_report_path_list.append(roc_mean_report_path)

        output_path = CONFIG.OUTPUT_DIRECTORY + 'nn_model_analysis/plot/metadata_' + metadata_metric + '/compare_nn_models/' + nn_model + '/'
    ...
    for metadata_metric in metadata_metric_list:
        ...
        for architecture in architecture_list:
            ...
            output_path = CONFIG.OUTPUT_DIRECTORY + 'nn_model_analysis/plot/metadata_' + metadata_metric + '/compare_nn_architecture/' + architecture + '/'
...
def main_plot_correlation_metrics(metric, mode):
    ...
    metadata_metric_list = ['NOT_USED', 'CORRELATION']
    mean_report_legends = {0: 'All Nodes', 1: 'Pearson'}
    ...
    for nn_model in nn_model_list:
        for architecture in architecture_list:
            ...
            output_path = CONFIG.OUTPUT_DIRECTORY + 'nn_model_analysis/plot/metadata_comparison/compare_nn_architecture/' + architecture + '/' + nn_model + '/'
