...
def generate_active_time_mean_per_node_per_date(data, time_step, active_mean_time, active_mean_time_rows):
    ...
    data_day = data.loc[(data['TIME'] >= datetime(date.year, date.month, date.day, 8, 0, 0)) &
                        (data['TIME'] < datetime(date.year, date.month, date.day, 20, 0, 0))].reset_index(drop=True)
    ...
    data_night = data.loc[(data['TIME'] < datetime(date.year, date.month, date.day, 8, 0, 0))].reset_index(drop=True)
    if len(data_night) != 0:
        sum_active = 0
        sum_not_active = 0
    ...
    data_night = data.loc[(data['TIME'] > datetime(date.year, date.month, date.day, 20, 0, 0))].reset_index(drop=True)
    if len(data_night) != 0:
        sum_active = 0
        sum_not_active = 0
        before = data_night['ACTIVE'][0]
    ...
    tmp_df = pd.DataFrame({'NODE': [node], 'DATE': date, 'TIME': ['DAY'],
    ...
    tmp_df_2 = pd.DataFrame({'NODE': [node], 'DATE': date, 'TIME': ['NIGHT'],
...
def generate_active_time_mean_all(data, time_step, active_mean_time, active_mean_time_rows):
    ...
    tmp_df = pd.DataFrame({'NODE': [node], 'TIME': ['DAY'],
                           'ACTIVE_MEAN': [statistics.mean(day_active_list)],
                           'NOT_ACTIVE_MEAN': [statistics.mean(day_not_active_list)],
                           'ACTIVE': [day_active_list], 'NOT_ACTIVE': [day_not_active_list]})
    tmp_df_2 = pd.DataFrame({'NODE': [node], 'TIME': ['NIGHT'],
                             'ACTIVE_MEAN': [statistics.mean(night_active_list)],
                             'NOT_ACTIVE_MEAN': [statistics.mean(night_not_active_list)],
                             'ACTIVE': [night_active_list], 'NOT_ACTIVE': [night_not_active_list]})
    active_mean_time = pd.concat([active_mean_time, tmp_df, tmp_df_2])
...
def plot_active_time_mean_per_node(dataset_path_list, output_path, legends, colors):
    ...
    for index, dataset_path in enumerate(dataset_path_list):
        ...
        for node in nodes:
            ...
            tmp_df = pd.DataFrame({'NODE': [node, node],
                                   'TIME': ['DAY', 'NIGHT'],
    ...
    for index, plot_data in enumerate(plot_data_list):
        plt_temp.append(list(plot_data.loc[plot_data['TIME'] == 'DAY']['ACTIVE_MEAN'].values))
    ...
    plt_temp = []
    for index, plot_data in enumerate(plot_data_list):
        plt_temp.append(list(plot_data.loc[plot_data['TIME'] == 'DAY']['NOT_ACTIVE_MEAN'].values))
    plt.hist(plt_temp, bins=20, color=colors, label=legends, density=True)
    ...
    plt_temp = []
    for index, plot_data in enumerate(plot_data_list):
        plt_temp.append(list(plot_data.loc[plot_data['TIME'] == 'NIGHT']['ACTIVE_MEAN'].values))
    plt.hist(plt_temp, bins=20, color=colors, label=legends, density=True)
    ...
    plt_temp = []
    for index, plot_data in enumerate(plot_data_list):
        plt_temp.append(list(plot_data.loc[plot_data['TIME'] == 'NIGHT']['NOT_ACTIVE_MEAN'].values))
    plt.hist(plt_temp, bins=20, color=colors, label=legends, density=True)
    # plt.ylim([0, 700])
...
def main_generate_active_time_per_node_per_date(benign_dataset_path, output_path, time_step):
    ...
    p = Pool(26)
    p.starmap(generate_active_time_mean_per_node_per_date, product(dataset_list, [time_step], [active_mean_time],
...
def main():
    ...
    for num_nodes in num_nodes_list:
        ...
        output_path = CONFIG.OUTPUT_DIRECTORY + 'stats/nodes_active_mean_time/' + str(num_nodes) + '/data/'
        prepare_output_directory(output_path)
        main_generate_active_time_all(benign_dataset_path, output_path, time_step)
        active_mean_time_all_dataset_path_list.append(output_path + 'active_mean_time_all.csv')
        main_generate_active_time_per_node_per_date(benign_dataset_path, output_path, time_step)
        active_mean_time_per_node_per_date_dataset_path_list.append(output_path + 'active_mean_time_per_node_per_date.csv')

    output_path = CONFIG.OUTPUT_DIRECTORY + 'stats/nodes_active_mean_time/plot_per_entry/'
    prepare_output_directory(output_path)
    plot_active_time_mean_per_entry(active_mean_time_all_dataset_path_list, output_path, legends, colors)
    output_path = CONFIG.OUTPUT_DIRECTORY + 'stats/nodes_active_mean_time/plot_per_node/'
    prepare_output_directory(output_path)
    plot_active_time_mean_per_node(active_mean_time_all_dataset_path_list, output_path, legends, colors)
