...
def plot_compare_benign_attack_packet_volume(benign_data_path, attack_data_path, output_path):
    ...
    plt.clf()
    plt.hist(benign_packets, bins=50, density=True, label='Benign - Average = ' + benign_mean)
    ...
    plt.clf()
    plt.hist(attack_packets, bins=50, density=True, label='Attack - Average = ' + attack_mean)
    plt.xlabel('Packet Volume')
    plt.ylabel('Probability Density')
    plt.legend()
...
def main():
    ...
    attack_data_path = CONFIG.DATASET_DIRECTORY + 'N_BaIoT/SimpleHome_XCS7_1003_WHT_Security_Camera_mirai_attack_udp_aggregation_Source-IP_time_window_10-sec_stat_Number.csv'
    output_path = CONFIG.OUTPUT_DIRECTORY + 'N_BaIoT/Output/Plot/'
    plot_compare_benign_attack_packet_volume(benign_data_path, attack_data_path, output_path)
