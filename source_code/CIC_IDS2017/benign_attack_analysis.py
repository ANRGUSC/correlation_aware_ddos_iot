...
def plot_features_benign_attack(input_dataset_path, output_path):
    ...
    for attack_type in attack_types_list:
        ...
        for feature in features_list:
            ...
            plt.title(feature_str)
            plot_output_path = output_path + 'attack/' + attack_type_str + '/attack_' + feature_str + '.png'
            prepare_output_directory(plot_output_path)
            plt.savefig(plot_output_path, bbox_inches='tight')
            ...
            plt.hist(plot_data_benign[feature], bins=50, density=True, label='Benign - Mean = ' + str(benign_mean))
            plt.hist(plot_data_attack[feature], bins=50, density=True, label='Attack - Mean = ' + str(attack_mean))
            plt.title(feature_str)
            plt.legend()
            plot_output_path = output_path + 'compare/' + attack_type_str + '/' + feature_str + '.png'
            prepare_output_directory(plot_output_path)
            plt.savefig(plot_output_path, bbox_inches='tight')
...
def plot_compare_feature_benign_attack(input_dataset_path, feature, attack_type, xlabel, ylabel, output_path):
    ...
    plt.clf()
    plt.hist(data_benign[feature] / math.pow(10, 6), bins=50, density=True, label='Benign - Average = ' + benign_mean)
    plt.hist(data_attack[feature] / math.pow(10, 6), bins=50, density=True, label='Attack - Average = ' + attack_mean)
...
def main():
    ...
    plot_compare_feature_benign_attack(input_dataset_path, ' Flow Duration', 'DoS Slowhttptest',
                                       'Flow Duration (Seconds)', 'Probability Density', output_path)

    output_path = output_directory + 'flow_iat_max.png'
    plot_compare_feature_benign_attack(input_dataset_path, ' Flow IAT Max', 'DoS Slowhttptest',
                                       'Maximum Flow Inter Arrival Time (Seconds)', 'Probability Density', output_path)

    output_path = output_directory + 'flow_iat_min.png'
    plot_compare_feature_benign_attack(input_dataset_path, ' Flow IAT Min', 'DoS Slowhttptest',
                                       'Minimum Flow Inter Arrival Time (Seconds)', 'Probability Density', output_path)

    output_path = output_directory + 'flow_iat_std.png'
    plot_compare_feature_benign_attack(input_dataset_path, ' Flow IAT Std', 'DoS Slowhttptest',
                                       'Standard Deviation Flow Inter Arrival Time (Seconds)', 'Probability Density', output_path)

    output_path = output_directory + 'flow_iat_mean.png'
    plot_compare_feature_benign_attack(input_dataset_path, ' Flow IAT Mean', 'DoS Slowhttptest',
